...
// 监听接收的数据
watch(() => store.receiveData, (newVal) => {
  if (newVal) {
    try {
      const regex = /}{/g;
      const separatedData = newVal.replace(regex, '}&&{').split('&&');
      separatedData.forEach(item => {
        const parseMsg = parseJSON(item)
...
<template>
  <div class="home">
    <div class="main-c">
      <div v-scroll.isAuto="true" ref="contentRef" class="main-c-content" v-if="store.$state.sendData.length">
